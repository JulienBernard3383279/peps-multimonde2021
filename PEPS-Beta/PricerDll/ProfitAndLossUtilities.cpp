...
void ProfitAndLossUtilities::updateDeltas(PnlVect * asset_deltas, PnlVect* fx_deltas, const PnlVect * FXRate, const PnlVect * spotsVect, const double port_value)
{
	...
	pnl_vect_div_vect_term(fx_deltas, FXRate);
...
void ProfitAndLossUtilities::updatePortfolioValue(double& port_value, double timestep, const PnlVect* asset_deltas, PnlVect* fx_deltas, const PnlVect * FXRate, const PnlVect * spotsVect)
{
	PnlVect* prices_in_euro = pnl_vect_copy(spotsVect);
	pnl_vect_mult_vect_term(prices_in_euro, FXRate);

	int nbAssets = fx_deltas->size;
	PnlVect* wonValue = pnl_vect_create_from_zero(nbAssets);
	for (int i = 0; i < nbAssets; ++i) {
		LET(wonValue, i) = exp(GET(FXRate, i)*timestep);
	}
	pnl_vect_mult_vect_term(fx_deltas, wonValue);
