...
namespace PricerDll.CustomTests
{
    public static unsafe class TestsQuanto
    {
        private static double RealPriceQuanto(
            ...
            double date)
        {
            double d1 = (Math.Log(currents[0] / strike) + (interestRates[0] + correlations[1] * volatilities[0] * volatilities[1] + 0.5 * volatilities[0] * volatilities[0])) / (volatilities[0] * Math.Sqrt(maturity - date));
